...
Recorder.prototype.gauge = function gauge(name, initialValue, tags) {
    ...
    const value = initialValue || 0;
    const adds = self._counterFactory(
        self, name + self.separator + 'adds', value, tags);
    const removes = self._counterFactory(
        self, name + self.separator + 'removes', value, tags);

    let previous = 0;

    const myGauge = {
        update: function(x) {
            // we don't update when the value is not changing
            if (previous < x) {
                adds.incr(x - previous);
            } else if (x < previous) {
                removes.incr(previous - x);
            }
            previous = x;
        }
    };
