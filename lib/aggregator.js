...
function Aggregator(recorder, _config) {
    ...
    const gaugeEventHandler = config.gauge ||
        DefaultConfig.aggregator.gauge;
    ...
    recorder.on('gauge', function onGauge(event) {
        gaugeEventHandler(event, self._histograms, self._counters,
        self._gauges);
    });
}
...
/**
 * Clear the aggregator of gauges states.
 *
 * @returns {null}.
 */
Aggregator.prototype.clearGauges = function clearGauges() {
    const self = this;
    self._gauges = {};
};
