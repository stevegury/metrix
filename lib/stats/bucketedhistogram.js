...
BucketedHistogram.prototype.add = function add(x) {
    ...
    if (x > self._maxValue) {
        ...
        const i = self._binarySearch(
            x, self._limits, 0, self._limits.length - 1);
...
BucketedHistogram.prototype.snapshot = function snapshot() {
    ...
    const targets = _.map(self._quantiles, function(q) {
        return (1.0 - q) * self._entries;
    });
    const results = new Array(self._quantiles.length);
    _.fill(results, 0);
    let resultIndex = results.length - 1;
    let n = self._overflow;
    ...
    for (let i = self._maxBucketId; i >= 0; i--) {
        n += self._buckets[i];

        // compute the result in backward order for the same reason.
        for (let j = resultIndex; j >= 0 ; j--) {
            if (n > targets[j]) {
                if (i === 0) {
    ...
    for (let k = 0; k < self._quantiles.length; k++) {
        const percentile = self._percentiles[k];
