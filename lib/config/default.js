...
const config = {
    recorder: {
        ...
        gauge: function(recorder, name, value, tags) {
            return new DefaultGauge(recorder, name, value, tags);
        }
    },
    aggregator: {
        timer: function(event, histograms, counters, gauges) {
            if (event.startTs) {
                event.value = event.stopTs - event.startTs;
                config.aggregator.histogram(event, histograms, counters);
            }
        },
        counter: function(event, histograms, counters, gauges) {
            if (!counters[event.name]) {
                counters[event.name] = 0;
            }
            counters[event.name] += event.increment;
        },
        histogram: function(event, histograms, counters, gauges) {
        ...
        gauge: function(event, histograms, counters, gauges) {
